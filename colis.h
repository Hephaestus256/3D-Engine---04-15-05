  ...
  (cylinder_3d cyl, point_3d pa, point_3d pb, point_3d ntersect)
{
  ...
  if (approx_equal (pa.z, pb.z))
    if (approx_equal (pa.x, pb.x)) {
      double bot_y = pb.y + cyl.height;
      double dy = bot_y - ntersect.y;
      p.y = pb.y - dy;
      p.x = pa.x;
      p.z = pa.z;
/*
...
{
//  _spec[15].abs = orig;
//  _spec[16].abs = *closest;
//  _spec[16].select = true;
   ...
   point_3d tent_top, point_3d tent_bot)
{
  ...
  if (path_int_plane_xyz (group, cyl, &result_a, &result_b, curr_top, curr_bot, tent_top, tent_bot, offset)) {
    closer = result_b;
//    bprint ("pen plane fwd", group.plane.sec_m1, 16);
//    _spec[8].abs = result_a;
//    _spec[8].select = true;
//    _spec[9].abs = result_b;
//    _spec[9].select = true;
    for (tri_type* tri = group.first; tri != NULL; tri = tri->next)
   ...
   point_3d tent_top, point_3d tent_bot)
{
  ...
  if (colis_xy_element_up (slide, &plane, result, group.plane, z_offset, pa, pb, tl, bl, br)) {
    store_closer (closest, *result, pa, plane, slide, res_plane);
    colis = true;    
  }
  if (colis_xy_element_dn (slide, &plane, result, group.plane, z_offset, pa, pb, br, tl, tr)) {
    store_closer (closest, *result, pa, plane, slide, res_plane);
    colis = true;
  }
...
///*
  if (_collis) {
    if (hit)
      tent = //closest;
      deflect_path (res_plane, cyl, closest, curr, tent);
